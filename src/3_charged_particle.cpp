...
)
{
    ...
    std::vector<float> visualized_field =
        std::vector<float>((n_r - 2 * sponge_width) * (n_r - 2 * sponge_width));
    ...
    for (int r = 0; r != iwidth; ++r)
    {
        for (int z = 0; z != iwidth; ++z)
        {
            ...
            const float sponge_sigma =
                ...
                std::pow(
                    std::max(
                        0.0f, 10.0f * (1.0f - sponge_r / (sponge_width * dr))
                    ),
                    2
                );
...
)
{
    ...
    for (int frame = 0; frame != n_t; ++frame)
    {
        ...
        std::vector<float> phi_next = iterate_potential(
            n_t, l_t, n_r, l_r, c, t, phi, phi_prev, true, sponge_width
        );
        phi_prev = phi;
        phi = phi_next;
...
int main(int, char **)
{
    ...
    const std::vector<std::vector<float>> visualized_fields =
        generate_visualized_field(n_t, l_t, n_r, l_r, c, sponge_width);

    // Visualizing the simulation.
    return visualize_field(visualized_fields, n_r - 2 * sponge_width);
}
